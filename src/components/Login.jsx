...
const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result.error) {
        console.error('❌ Error de login con Google:', result.error)
        setError(result.error.message)
      }
        ...
        <form className="mt-6 sm:mt-8 space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              ...
              disabled={loading}
              className="group relative w-full flex justify-center items-center py-2 sm:py-3 px-4 border border-gray-300 text-sm sm:text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
            >
